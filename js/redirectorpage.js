	...
	chrome.runtime.sendMessage({type: "save-redirects", redirects: arr}, function(response) {
		if (response && response.message) {
			...
			if(response.message.indexOf("Redirects failed to save") > -1){
				...
				console.log('Saved ' + arr.length + ' redirects at ' + new Date() + '. Message from service worker:' + response.message);
			}
		} else {
			console.log('Saved ' + arr.length + ' redirects at ' + new Date());
			// In MV3, sendMessage response may be undefined if service worker doesn't explicitly send a response
			chrome.storage.local.set({redirects: arr}, function() {
				console.log('Redirects saved to storage directly.');
			});
		}
	});
}
...
function toggleSyncSetting() {
	chrome.runtime.sendMessage({type: "toggle-sync", isSyncEnabled: !options.isSyncEnabled}, function(response) {
		if (response) {
			...
			// Direct storage update in case service worker doesn't respond
			options.isSyncEnabled = !options.isSyncEnabled;
			chrome.storage.local.set({isSyncEnabled: options.isSyncEnabled}, function() {
				showMessage(options.isSyncEnabled ? 'Sync is enabled!' : 'Sync is disabled - local storage will be used!', true);
			});
		}
...
function pageLoad() {
	...
	chrome.runtime.sendMessage({type: "get-redirects"}, function(response) {
		...
		// Only add example redirect if there are no redirects at all
